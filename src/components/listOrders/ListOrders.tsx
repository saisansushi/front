...
const ListOrder = (props: IListOrder) => {
    ...
    useEffect(() => {
        ...
        axios.post(props.apiURL + "order/get_list/",
            {
    ...
    const OrderList = () => {
        const orderItem = load && orders.map((order: { customer: { firstname: string, telephone: string }, dateAdded: string, orderId: number }) =>
            <StyledTableRow key={order.orderId} onClick={() => {
    ...
    return (
        <Wrapper>
            ...
            <Footer>
                <Container>
                    <Box m={1}> <Button variant="outlined" onClick={() => {
...
const mapDispatch = {
    setCurrentOrder: (orderId: number) => ({type: DT.currentOrder, openOrderId: orderId}),
