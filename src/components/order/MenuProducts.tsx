...
const MenuProducts = (props: IMenuProducts) => {
    ...
    const category = searchProduct.length === 0 && props.init.products.category.filter(({parentId}) => parentId === menuLevel[menuLevel.length - 1]).map(({
                                                                                                                                                              categoryId,
                                                                                                                                                              name
                                                                                                                                                          }) =>
        <BtnMenu key={categoryId} onClick={() => {
            setMenuLevel([...menuLevel, categoryId])
        }}><FolderOpen/><Typography>{name}</Typography></BtnMenu>
    )


    const filterProduct = searchProduct.length === 0 && props.init.products.product.filter(({categoryId}) => categoryId.indexOf(menuLevel[menuLevel.length - 1]) !== -1) || props.init.products.product.filter((e) => e.productName.toLowerCase().includes(searchProduct.toLowerCase()));


    const product = filterProduct.map(({productId, productName, price, options}) => {
            const required = (options.some(({required}) => required === 1));
            return (
                <BtnMenu block key={productId} onClick={() => {
                    if (options.length > 0) {
                        setSearchProduct("");
                    ...
                    {options.length > 0 && required ? <Menu/> : <Typography>{price + ' ₽'}</Typography>}
                </BtnMenu>
            )
        }
    )
    ...
    const changeOptionSelected = (opt: { productOptionValueId: number }) => {
        let newElem = true;
        let optionIdList = optionSelected.reduce((acc: any[], currentValue: { productOptionValueId: number; }) => {
    ...
    const options = (searchProduct.length === 0 && optionArr.current.length > 0) && optionArr.current[optionStep.current].optionValues.map((opt: { productOptionValueId: number; productId: number; optionValueName: string; optionPrice: number; pricePrefix: string; }) => {
            ...
            return (<BtnMenu
                selected={optionSelected.some((e: { productOptionValueId: number; }) => e.productOptionValueId === opt.productOptionValueId)}
                block
                key={opt.productOptionValueId} onClick={() => {
    ...
    const optionsBefore = () => {
        if (optionArr.current.length > 0 && optionArr.current[optionStep.current].required) {
            return <Alert severity={"warning"} style={{width: '80%', textAlign: 'center', marginTop: '20px'}}>Выберите
                опцию</Alert>
        } else if (optionArr.current.length > 0 && optionArr.current[optionStep.current].required === 0) {
            return <Alert severity={"warning"} style={{width: '80%', textAlign: 'center', marginTop: '20px'}}>Предложите
                клиенту добавки</Alert>
        }
    }

    const optionsAfter = () => {
        if (optionArr.current.length > 0 && optionArr.current[optionStep.current].required === 0) {
            return <div style={{width: '80%', textAlign: 'center', marginTop: '20px'}}>
                <Button fullWidth={true} variant="outlined" onClick={() => {
                    addProduct(menuLevel[menuLevel.length - 1], optionSelected);
                    menuLevelUp()
                }}>Закончить</Button></div>
        }
    }
    ...
    const addProduct = (id: number, options: IOption[] | never = []) => {
        ...
        const optionIdList = options && options.reduce((acc: any[], currentValue: { productOptionValueId: number; }) => {
            acc.push(currentValue.productOptionValueId);
            return acc
        }, []).sort();
        let selectedProduct = allOrderProduct.filter((e) => e.productId === id && e.optionIdList.toString() === optionIdList.toString())[0];
        // уникальный ключ продукта в заказе//  id продукта - id  обязательных опциий возрастанию -  id доп опций по возрастанию
        let key: string = id + (optionIdList.length > 0 ? '_' : '') + optionIdList.join('_')
        if (selectedProduct) {
            selectedProduct.quantity++;
            selectedProduct.total = selectedProduct.price * selectedProduct.quantity;
        } else {
            let product = props.init.products.product.filter(({productId}) => productId === id)[0];  // выбраный продукт в меню
            let price = optionIdList.length > 0 && getOptionPrice(options) + product.price || product.price;
    ...
    const buttonBackMenu = menuLevel[menuLevel.length - 1] === 0
        &&
        <Button variant="outlined" style={{width: '20%'}}
                onClick={() => {
    ...
    return (
        <div style={{display: 'flex', flexDirection: 'column', justifyContent: 'space-between', maxHeight: '100%'}}>
            <Grid container direction="row">
                <TextField variant="outlined" color="primary" style={{width: '80%'}} label="Поиск" type="search"
                           onChange={(event) => {
                               setSearchProduct(event.target.value)
                           }} value={searchProduct}/>
                {buttonBackMenu}
            </Grid>
            <Grid spacing={0} container direction="row" justify="space-around"
                  style={{overflowY: 'auto', maxHeight: '100%'}}>
                {category}{product}{optionsBefore()}{options}{optionsAfter()}
            </Grid>
        </div>
    )
}
...
const mapDispatch = {
    ...
    changeMenuMode: (menuMode: boolean) => ({type: DT.setMenuMode, menuMode: menuMode})
}
