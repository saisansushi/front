...
import {Wrapper, Main, Header, Footer, Preloader} from '../styledComponents/stilesComponents';
import {
    Grid,
    Button,
    Container
...
const OrderView = (props: IOrderView) => {
    ...
    const unlockAndExit = () => {unlockOrder().then(() => props.changePage("list"))}

    const unlockOrder = () => {
        showPreloader(true);
        const request = axios.post(props.apiURL + "order/unlock_order/",
            {
    ...
    const saveOrder = () => {
        showPreloader(true);
        const request = axios.post(props.apiURL + "order/save_order/",
            {
                terminalId: props.terminalId,
                orderId: props.openOrderId,
    ...
    useEffect(() => {
        ...
        axios.post(props.apiURL + "order/get_order/",
            {
                terminalId: props.terminalId,
                orderId: props.openOrderId
            }
    ...
    if (locked) {
        question = <Question title={"Заказ заблокирован терминалом - " + props.order.locked} body="Закрыть заказ"
                             callback={() => props.changePage("list")} typeButton={'ok'}/>  // на заблокированном заказе выходим без проверки на изменение
    ...
    const btnMenu = props.navigation.menuMode && <Button variant="outlined" style={{margin: '1px'}} onClick={() => {
        props.changeMenuMode(false)
    }}>Закрыть меню </Button> || <Button variant="outlined" style={{margin: '1px'}} onClick={() => {
        props.changeMenuMode(true)
    }}>Меню </Button>
    const leftPanel = props.navigation.menuMode && <MenuProducts/> || <CustomerFragment/>
    const preLoader = (preloader || !initLoad) && <Preloader/>;
    const view = initLoad && <Wrapper>
        <Header>
        ...
        <Main row>
            <Container>
                <Grid container spacing={3} style={{height: '100%'}}>
                    ...
                    <Grid item xs={7} style={{maxHeight: '100%', overflowY: 'scroll', display: 'flex'}}>
                        <ProductsFragment/>
                    </Grid>
        ...
        <Footer>
            ...
            {btnMenu}
            <Button variant="outlined" style={{margin: '1px'}} onClick={() => {
                saveOrder().then()
            }}>Сохранить </Button>
            <Button variant="outlined" style={{margin: '1px'}} onClick={() => {
                exitOfOrder()
