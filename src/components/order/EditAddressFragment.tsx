...
import {FullscreenControl, Map, Placemark, Polygon, YMaps, ZoomControl} from "react-yandex-maps";
import {
...
const EditAddressFragment = (props: IEditAddressFragment) => {
    ...
    const YMapsApikey="e9fcb0ac-fe84-4975-8325-a2cea341a5ff";
    const configHeaders = {'Content-Type': 'application/json', 'Token-Authorization-X': props.config.token}
    ...
    const initAddressEdit = (el: string) => {
        if (props.order.addresses.some(({addressId}) => addressId)) {
            return props.order.addresses.filter(({addressId}) => addressId === props.order.addressId)[0][el]
        }
        return null
    }

    const [city, setCity] = useState(() => edit ? initAddressEdit('city') : '');
    const [street, setStreet] = useState<string | null>(() => edit ? initAddressEdit('street') : null);
    const [house, setHouse] = useState<string | null>(() => edit ? initAddressEdit('house') : null);
    const [geoLat, setGeoLat] = useState<number | null>(() => edit ? initAddressEdit('geoLat') : null);
    const [geoLon, setGeoLon] = useState<number | null>(() => edit ? initAddressEdit('geoLon') : null);
    const [polygonId, setPolygonId] = useState(() => edit ? initAddressEdit('polygonId') : 0);
    const [qcGeo, setQcGeo] = useState<number | null>(() => edit ? initAddressEdit('qcGeo') : null);//точность координат 0- точные  1 - Ближайший дом 2 - Улица 3 - Населенный пункт 4 - Город 5 - Координаты не определены
    const [fiasLevel, setFiasLevel] = useState<number | null>(() => edit ? initAddressEdit('fiasLevel') : null);//точность адреса 0 — страна 1 — регион 3 — район  4 — город  5 — район города   6 — населенный пункт   7 — улица   8 — дом   65 — планировочная структура   90 — доп. территория   91 — улица в доп. территории  -1 — иностранный или пустой
    const [address, setAddress] = useState<string | null>(() => edit ? initAddressEdit('addressRepresent') : null)
    const [polygonDescription, setPolygonDescription] = useState<string | null>(() => edit ? initAddressEdit('polygonDescription') : null);
    const [addressChecked, setAddressChecked] = useState<boolean>(false);
    const [room, setRoom] = useState<string | null>(() => edit ? initAddressEdit('room') : null);
    const [porch, setPorch] = useState<string | null>(() => edit ? initAddressEdit('porch') : null);
    const [floor, setFloor] = useState<string | null>(() => edit ? initAddressEdit('floor') : null);
    const [comment, setComment] = useState<string | null>(() => edit ? initAddressEdit('comment') : null);
    const [error, setError] = useState<string | null>('');

    const YM = useRef({});
    //const [cord, setCords ] =useState([45.002434, 38.94184]);

    const selectSuggestion = (suggestion: DaDataSuggestion<DaDataAddress> | undefined) => {
        ...
        setGeoLat(suggestion.data.geo_lat ? parseFloat(suggestion.data.geo_lat) : null);
        setGeoLon(suggestion.data.geo_lon ? parseFloat(suggestion.data.geo_lon) : null);
    ...
    const makeAddressString = (suggestion: DaDataSuggestion<DaDataAddress>) => {
        ...
        if ((suggestion.data.region || suggestion.data.area) && !suggestion.data.city && !suggestion.data.settlement) {
            return '';
        }
        return joinCustom([suggestion.data.city_with_type,
            suggestion.data.settlement_with_type,
            suggestion.data.street_with_type,
            joinCustom([suggestion.data.house_type, suggestion.data.house,
    ...
    const postNew = (data: object) => {
        ...
        axios.post(props.config.apiURL + "address/newAddress/", data
            , {
    ...
    const postUpdate = (data: object) => {
        props.setPreloader(true);
        axios.post(props.config.apiURL + "address/updateAddress/", data
            , {
                headers: configHeaders
            }
        )
            .then(response => {
                ...
                let newAddresses = response.data.address.addresses;
                if (newAddresses) {
                    props.orderSetContent({addresses: newAddresses, shippingMethod: 'delivery'})
                    props.setAddressMode('none')
                }
    ...
    const getPolygon = () => {
        props.setPreloader(true);
        axios.post(props.config.apiURL + "address/get_polygon/",
            {
                terminalId: props.appState.terminalId,
                geoLat: geoLat,
                geoLon: geoLon
    ...
    const yMap = <YMaps query={{apikey: YMapsApikey}}>
        <Map modules={["geolocation", "geocode"]}
             state={{
            ...
            <Placemark geometry={[geoLat ? geoLat : pointLat, geoLon ? geoLon : pointLon]}
                       options={{
                           draggable: "true",
                       }}
                       properties={{
            ...
            />
            <ZoomControl/> <Polygon/><FullscreenControl/>
        </Map>

    </YMaps>


    const addressRepresent = [address,
        (address && room) ? ", кв " + room : "",
        (address && porch) ? ", п " + porch : "",
        (address && floor) ? ", э " + floor : ""].join('');

    const message = (polygonId === 0) && <></> || (polygonId === -1) &&
    ...
    return (
        ...
        <Dialog open={true} maxWidth={'lg'}>
            <DialogTitle> {props.navigation.addressMode === 'new' ? 'Добавление адреса' : 'Редактирование адреса'}</DialogTitle>
            <DialogContent>
                ...
                <Grid item xs={12} style={{padding: '10px'}}>
                    ...
                    <AddressSuggestions
                        ...
                        filterLocations={[{area_fias_id: "34a0d847-a43b-421f-86c3-8eaf84c1ce28"}]}/>
                ...
                <Grid container>
                    <Grid item xs={4} style={{padding: '10px'}}>
                        <TextField fullWidth label="Квартира"
                                   onChange={(e) => setRoom(e.target.value)} value={room ? room : ''}/>
                    </Grid>
                    <Grid item xs={4} style={{padding: '10px'}}>
                        <TextField fullWidth label="Подъезд"
                                   onChange={(e) => setPorch(e.target.value)} value={porch ? porch : ''}/>
                    </Grid>
                    <Grid item xs={4} style={{padding: '10px'}}>
                        <TextField fullWidth label="Этаж" onChange={(e) => setFloor(e.target.value)}
                                   value={floor ? floor : ''}/>
                    </Grid>

                    <Grid item xs={12} style={{padding: '10px'}}>
                        <TextField fullWidth label="Комментарий"
                                   onChange={(e) => setComment(e.target.value)} value={comment ? comment : ''}/>
                    </Grid>
                </Grid>
                {errorMessage}
                {message}
            ...
            <DialogActions>
                ...
                <Button color="primary" variant="outlined"
                        onClick={clickButtonOK}>{addressChecked ? "Сохранить" : "Проверить"}
                </Button>
            </DialogActions>
        </Dialog>
    )
...
const mapDispatch = {
    orderSetContent: (content: object) => ({type: DT.orderSetContent, content: content}),
    setAddressMode: (mode: string) => ({type: DT.setAddressMode, addressMode: mode}),
    setNetworkError: (action: boolean) => ({type: DT.setNetworkError, networkError: action}),
    setPreloader: (action: boolean) => ({type: DT.setPreloader, preloader: action})
}
