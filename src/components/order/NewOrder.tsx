...
const NewOrder = (props: INewOrder) => {
    ...
    const cbGetVendor = (vendorId: number | null, locationVerified: boolean = false) => {
        if (vendorId) {
            // запрашиваем вендора переходим к запросу телефона
            newOrderData.current = {vendorId: vendorId, locationVerified: locationVerified}
    ...
    const checkCustomer = (phone: string) => {
        ...
        axios.post(props.apiURL + "customer/check_customer_by_phone/", {
            ...
            .then(response => {
                ...
                if (customerArr.customerId) {
                    ...
                    newOrderData.current = {...newOrderData.current, 'customerId': customerArr.customerId}
                    createOrder();
                } else {
    ...
    const createOrder = () => {
        showPreloader(true);
        // создание заказа
        axios.post(props.apiURL + "order/create_order/", {
                terminalId: props.terminalId,
    ...
    switch (receptionStep) {
        case 'vendor':
            ...
            break;
        case 'phone':
            view = <InputPhone cbNumberInput={cbGetPhone} maxLengthNumberInput={10} placeholder={"9000000000"}
                               title={"Телефон"}/>;
            break;
        case'name':
        ...
        case'city':
            view =
                <Question title="Вопрос клиенту!" body="В каком населенном пункте вы находитесь?"
                          callback={cbQuestionCity} footer={'Наш клиент?'} typeButton={'no-yes'}/>;
            break;
    }

    if (questionSave) {
        view =
            <Question title="Оформление не закончено!" body="Выйти?" callback={cbQuestionExit} typeButton={'yes-no'}/>
    } else if (preloader) {
        view = <Preloader/>
    }
    return (
...
const mapDispatch = {
    currentOrder: (orderId: number) => ({type: DT.currentOrder, openOrderId: orderId}),
