...
const CustomerFragment = (props: ICustomerFragment) => {
  ...
  const clientAddressesItem = props.order.addresses.map(
    ({ addressId, addressRepresent }) => (
      <DivCell
        key={addressId}
        style={{ width: "100%" }}
  ...
  const clientAddresses = viewSelectAddress && (
    <Grid container>
      <DivCell
        style={{
    ...
    <Grid>
      <Grid container direction="row">
        <ButtonGroup style={{ width: "100%" }}>
          ...
          <Button variant="outlined" style={{ width: "10%" }}>
            i
          </Button>
          <Button variant="outlined" style={{ width: "10%" }}>
            i
      ...
      <Grid container direction="row">
        <ButtonGroup style={{ width: "100%" }}>
          <DivCell style={{ width: "80%" }}>
            <Typography variant="body1">
              {props.order.customer.telephone}{" "}
            </Typography>
          </DivCell>
          <Button variant="outlined" style={{ width: "10%" }}>
            i
      ...
      <Grid container direction="row">
        <ButtonGroup style={{ width: "100%" }}>
          <DivCell
            style={{ width: "90%" }}
            onBlur={() => {
              setTimeout(() => setViewSelectAddress(!viewSelectAddress), 200);
            }}
          ...
          <Button
            ...
            variant="outlined"
            onClick={() => {
              if (props.order.shippingMethod !== "pickup") {
                props.changeAddressMode("edit");
              }
            }}
          >
            <Edit />
      ...
      <div style={{ position: "relative" }}>
        <BlockSelectAddress>{clientAddresses} </BlockSelectAddress>
      </div>

      <Grid container direction="row">
        <Textarea
          rows={3}
          value={props.order.comment ? props.order.comment : ""}
...
const mapDispatch = {
  orderSetContent: (content: IOrderSetContent) => ({
    type: DT.orderSetContent,
    content: content,
  }),
  changeAddressMode: (mode: string) => ({
