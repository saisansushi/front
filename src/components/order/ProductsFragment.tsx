...
const ProductsFragment = (props: IProductsFragment) => {
  ...
  const removeProduct = () => {
    let product = Object.assign(
      [],
      props.order.products.filter(
        ({ productKey }) => productKey !== editProductProductKey
      )
    );
  ...
  const changeQuantity = (op: string) => {
    let product = props.order.products.filter(
      ({ productKey }) => productKey === editProductProductKey
    )[0]; // мутирую стор
    if (op === "up") {
      product.quantity++;
  ...
  const optionList = (optionList: []) => {
    ...
    let li = optionList
      ...
      .map(({ optionValueId, optionValueName, optionName }) => (
        <li style={{ fontSize: "0.7rem" }} key={optionValueId}>
  ...
  const optionListRequired = (optionList: []) => {
    ...
    let options = optionList
      ...
      .map(({ optionValueId, optionValueName }) => (
  ...
  const productItem = props.order.products.map((opt) => {
    return (
      <StyledTableRow
        key={opt.productKey}
        onClick={(e) =>
          clickProduct(opt.productKey, opt.productName, opt.quantity, e)
        }
      >
        <StyledTableCell>
          {opt.productName} {optionListRequired(opt.optionList)}{" "}
          {optionList(opt.optionList)}{" "}
        </StyledTableCell>
    ...
    <>
      <TableContainer>
        <Table stickyHeader>
          ...
          <TableBody>{productItem}</TableBody>
        </Table>
      </TableContainer>
      <StyledMenu
        keepMounted
